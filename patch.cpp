...
void CPatch::AddrHook(uintptr_t addr, uintptr_t func, uintptr_t *orig)
{	
	CPatch::ReadMemory(mmap_start, addr, 16);
	*orig = mmap_start;

	CPatch::WriteHookProc(addr, func);
	mmap_start += 16;
}
